...
const Home = () => {
  ...
  const handleFecthData = async () => {
    try {
      ...
      const response = await apiConnector('GET', HOME_API, null, {
    ...
    return (
      <div className='w-11/12 relative mt-10 grid  grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3  '>
      {
        newsData?newsData.map((data,index)=>(
          <div key={index} className=' text-2xl border shadow-richblack-300 shadow-inner opacity-90 bg-white rounded-md min-h-[20rem] p-2'>
            <div>
